...
- По умолчанию генератор создает 700 пользователей (config.userCount = 700)
...
- В среднем: 700 пользователей × 3 сессии (среднее) × 6 событий (среднее) = примерно 12600 событий
...
## 8. Переход на WebClient для улучшения производительности

### Текущая проблема:
Текущая реализация использует `HttpURLConnection` для выполнения HTTP-запросов в шлюзах `AnalyticsGateway` и `TrackingGateway`. Этот подход имеет следующие ограничения:
- Синхронная блокирующая операция
- Отсутствие пула соединений
- Нет встроенной поддержки повторных попыток, таймаутов и обработки ошибок
- Менее эффективное использование ресурсов

### Решение:
Для улучшения производительности и надежности системы были созданы новые реализации шлюзов с использованием `WebClient`:
- `WebClientAnalyticsGatewayImpl` - заменяет `AnalyticsGatewayImpl`
- `WebClientTrackingGatewayImpl` - заменяет `TrackingGatewayImpl`

### Преимущества WebClient:
- **Пул соединений**: Автоматическое управление пулом соединений для повторного использования
- **Улучшенная обработка ошибок**: Встроенные механизмы обработки HTTP-ошибок
- **Таймауты**: Возможность настройки таймаутов для запросов
- **Более чистый API**: Более современный и удобный API для выполнения HTTP-запросов
- **Лучшая интеграция**: Лучшая интеграция с экосистемой Spring
- **Будущая совместимость**: Возможность перехода на полностью реактивную архитектуру в будущем